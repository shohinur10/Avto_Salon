...
const CarCard = (props: CarCardType) => {
	...
	if (device === 'mobile') {
		...
		return (
			<Box className="car-card" sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
				{/* Card Header with Image */}
				<Box className="card-header" sx={{ position: 'relative', borderRadius: '12px 12px 0 0', overflow: 'hidden' }}>
					<Link
						href={{
							pathname: '/car/detail',
							query: { id: car?._id },
				...
				<Box className="card-body" sx={{ 
					...
					<Box className="car-info" sx={{ marginBottom: '16px' }}>
													<Link
							href={{
								pathname: '/car/detail',
								query: { id: car?._id },
					...
					{/* Car Specifications */}
					<Box className="car-specs" sx={{ marginBottom: '16px' }}>
						<Box sx={{ 
							display: 'grid', 
					...
					<Box className="car-status" sx={{ marginTop: 'auto' }}>
						<Box sx={{ 
							display: 'flex', 
							...
							{/* Views and Likes */}
							<Box sx={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#999' }}>
								{car.carViews && (
									<Tooltip title="Views">
