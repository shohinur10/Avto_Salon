...
import PorscheStyleCards from './PorscheStyleCards';
import 'swiper/css';
...
const PopularCars: React.FC<PopularCarsProps> = ({ cars = [], onLikeToggle }) => {
	...
	return (
		...
		>
			...
			{/* Use Porsche-style cards for the first 3 cars */}
			<PorscheStyleCards cars={cars} onLikeToggle={onLikeToggle} />

			{/* Traditional carousel for additional cars if more than 3 */}
			{cars.length > 3 && (
				<motion.div
					initial={{ opacity: 0, scale: 0.9 }}
					animate={isInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.9 }}
					transition={{ duration: 0.8, delay: 0.5 }}
					className="additional-cars-container"
				>
					<Box component="div" className="section-header">
						<Typography variant="h3" className="additional-cars-title">
							More Popular Cars
						</Typography>
					</Box>
					
					<Box component="div" className="cars-carousel-container">
						<Swiper
							spaceBetween={30}
							slidesPerView={1}
							navigation={{
